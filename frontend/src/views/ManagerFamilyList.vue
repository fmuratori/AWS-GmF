...
b-container.justify-content-center.my-5
  ...
  b-row
    b-col(v-if="familyList.length == 0", sm=12, md=6)
      p Non hai mai effettuato segnalazioni. Premi #[a(href="#", @click="$router.replace({ name: 'ManagerFamilySubscribe' })") qui] per segnalare una famiglia bisognosa.

    b-col(v-else, sm=12, md=6, v-for="(family, idx) in familyList", :key="idx")
      b-card.mb-4(bg-variant="light", text-variant="dark", no-body)
        b-card-text
          b-card-header 
            h5 
              b {{ family.name }}
          ...
          b-button.b-card-footer-button(
            ...
            variant="success",
            @click="$router.replace({ name: 'ManagerFamilySubscribe', params: { family: family } })"
          ) EDIT
